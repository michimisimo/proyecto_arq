...
async function obtenerDestinatarios() {
    try {
        const response = await axios.get('http://service-gest-dest-b:3010/api/dest/getDest');
...
async function crearEnvio(idDif) {
    try {
        const response = await axios.post('http://service-env-email-b:3013/api/email/createEnv/' + idDif);
        return response.data;
    } catch (error) {
