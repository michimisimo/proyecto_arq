...
async function obtenerDestinatarios() {
    try {
        const response = await axios.get('http://service-gest-dest-a:3000/api/dest/getDest');
...
async function crearEnvio(idDif) {
    try {
        const response = await axios.post('http://service-env-email-a:3003/api/email/createEnv/' + idDif);
        return response.data;
    } catch (error) {
