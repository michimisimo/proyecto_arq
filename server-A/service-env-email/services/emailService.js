...
async function getDifusionCampana(id_campana) {
    try {
        const response = await axios.get('http://service-gest-dif-a:3002/api/dif/getDestDif/' + id_campana);
...
async function getDestinatariosCampana(id_campana) {
    try {
        const response = await axios.get('http://service-gest-dif-a:3002/api/dif/getDetDif/' + id_campana);
        return response.data;
    } catch (error) {
        console.error('Error al obtener los destinatarios de difusión:', error);
        throw error;
...
exports.iniciarEnvioCampana = async (campana) => {
    ...
    try {
        ...
        await axios.patch(`http://service-gest-cam-a:3001/api/cam/updateEstadoCampana/${campana.id_campana}`, { id_estado: 2 });

        // Obtener remitente, asunto y contenido de correo
        const emailData = await axios.get(`http://service-gest-cam-a:3001/api/cam/getEmailCampana/${campana.id_campana}`);
        ...
        if (reporte.correos_enviados > 0) {
            ...
            await axios.patch('http://service-gest-cam-a:3001/api/cam/updateEstadoCampana/' + campana.id_campana, { id_estado: 1 });
        } else {
            console.log("No se enviaron correos, cambiando estado a fallida.");
            await axios.patch('http://service-gest-cam-a:3001/api/cam/updateEstadoCampana/' + campana.id_campana, { id_estado: 4 });
        }
        try {
            console.log("Almacenando reporte final...");
            const response = await axios.post('http://service-reportes-a:3004/api/rep/saveReporte', {
                ...reporte,
        ...
        await axios.post('http://service-gest-cam-a:3001/api/cam/updateEstadoCampana/' + campana.id_campana, { id_estado: 4 });
    }
};
